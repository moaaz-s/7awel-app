...
const SIGNUP_FLOW_STEPS: FlowStep[] = [
  ...
  { step: AUTH_STEP_USER_PROFILE_PENDING, condition: ctx => ctx.tokenValid },
...
const SIGNIN_FLOW_STEPS: FlowStep[] = [
  ...
  { step: AUTH_STEP_PIN_SETUP_PENDING, condition: ctx => ctx.tokenValid && !ctx.pinSet },
  { step: AUTH_STEP_PIN_ENTRY_PENDING, condition: ctx => ctx.tokenValid && ctx.pinSet && !ctx.sessionActive },
  { step: AUTH_STEP_AUTHENTICATED },
];
...
export const LOCKED_FLOW: AuthStep[] = [AUTH_STEP_PIN_ENTRY_PENDING, AUTH_STEP_AUTHENTICATED];
