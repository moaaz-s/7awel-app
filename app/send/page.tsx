...
export default function SendMoneyPage() {
  const { contacts, balance, formatCurrency } = useApp()
  const { t } = useLanguage()
      ...
      {step === "amount" && state.recipient && (
        <div className="flex flex-col items-center space-y-6">
          ...
          <AmountInput
            ...
            availableBalance={t("transaction.available", { amount: formatCurrency(balance) })}
          />
        </div>
      )}

      {step === "confirmation" && state.recipient && (
        <div className={spacing.section}>
          {error && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">{error}</div>}

          <CardContainer
            header={<h3 className="text-center text-lg font-medium">{t("transaction.paymentDetails")}</h3>}
          >
            <div className={spacing.stack}>
          ...
          <ButtonPrimary onClick={handleConfirm} disabled={isLoading} loading={isLoading} fullWidth size="xl">
            {t("transaction.send", { amount: state.amount })}
          </ButtonPrimary>
        </div>
      )}
    </PageContainer>
