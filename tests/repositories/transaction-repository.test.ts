...
describe("TransactionRepository integration", () => {
  ...
  it("listLocal returns sorted transactions", async () => {
    ...
    const unsorted: Transaction[] = [
      { id: "1", amount: 10, type: "DEBIT", status: "pending", assetSymbol: "USD", createdAt: new Date(now - 1000).toISOString() } as any,
      { id: "2", amount: 20, type: "CREDIT", status: "completed", assetSymbol: "USD", createdAt: new Date(now).toISOString() } as any,
    ];
  ...
  it("listRemote returns items and caches top 10 to local", async () => {
    const sample = Array.from({ length: 12 }).map((_, i) => ({ id: `r${i}`, amount: i + 1, type: "DEBIT", status: "pending", assetSymbol: "USD", createdAt: new Date().toISOString() } as any));
  ...
  it("getTransaction falls back to remote and caches minimal subset", async () => {
    getTransactionByIdMock.mockResolvedValue({ statusCode: 200, data: { id: "tx9", amount: 99, type: "DEBIT", status: "pending", assetSymbol: "USD", createdAt: new Date().toISOString() } });
